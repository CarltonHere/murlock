...
export function MurLock(
  releaseTime: number,
  keyParams: string[],
  waitTime?: number | ((retries: number) => number)
) {
  return (target: any, propertyKey: string, descriptor: PropertyDescriptor) => {
    ...
    descriptor.value = async function (...args: any[]) {
      ...
      const murLockService: MurLockService = Reflect.getMetadata(
        MURLOCK_SERVICE_METADATA_KEY,
        MurLockService
      )
      if (!murLockService) {
      ...
      await acquireLock(lockKey, murLockService, releaseTime, waitTime)
      try {
...
async function acquireLock(
  ...
  try {
    isLockSuccessful = await murLockService.lock(lockKey, releaseTime, waitTime)
  } catch (error) {
    throw new MurLockException(
